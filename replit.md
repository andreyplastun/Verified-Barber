...
- **Guest Client Identity System**: Supports bookings without phone numbers via `altegio_client_id` and `normalized_phone` fields. Identity priority: 1) `altegio_client_id` (highest), 2) `normalized_phone`, 3) guest fallback. Merge safety: only merge when phones match AND altegio_client_ids are identical or absent. Phone-appeared-later updates `is_guest=false` without auto-merging conflicting identities. Structured logging: `[CLIENT_CREATED_GUEST]`, `[CLIENT_UPDATED_PHONE]`, `[CLIENT_IDENTITY_CONFLICT]`, `[PHONE_MATCH_EXISTING_ALTEGIO_CLIENT]`. Helper: `normalizePhone()` in `server/client-identity.ts`. DB indexes on `altegio_client_id` and `normalized_phone`.
