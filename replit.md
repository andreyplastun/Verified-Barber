...
- **Review Weight System (Этап 1)**: Multi-factor review weighting stored in `review_geodata`. Factors: `geo_weight` (distance-based), `new_weight` (unique clients per 7 days: ≥3→1.0, 2→0.85, ≤1→0.6), `repeat_weight` (1 review per phone per specialist per 60 days, else 0). `final_weight = geo_weight * new_weight * repeat_weight` for manual reviews; for Altegio reviews `final_weight = 1.0` (only `repeat_weight` applied). Rating calculation in `updateSpecialistRating` uses `finalWeight` from geodata. Functions: `calculateNewWeight()`, `calculateRepeatWeight()` in `server/antifraud.ts`.
- **Geofencing System**: Location-based review trust via `locations`, `specialist_locations`, `review_geodata` tables. Haversine distance calculation against specialist's linked locations. Geo weight: ≤150m=1.0, ≤500m=0.6, >500m=0.2, no_permission=0.5. Time penalty (>2h from visit → ×0.3), IP penalty (≥5/hour → ×0.7). Admin CRUD at `/api/admin/locations`. Frontend geolocation request on MagicReviewPage with trust badge.
