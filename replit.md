...
- **Geofencing System**: Location-based review trust via specialist's own `workAddress`/`workLat`/`workLng` fields. Specialist sets address in profile (text input + geocode, or "determine my location" GPS button). Nominatim (OpenStreetMap) geocoding. Haversine distance from reviewer to specialist's work address. Geo weight: ≤150m=1.0, ≤500m=0.6, >500m=0.2, no_permission=0.5. Time penalty (>2h from visit → ×0.3), IP penalty (≥5/hour → ×0.7). Frontend geolocation request on MagicReviewPage with trust badge. Admin "Точки" tab removed — no manual location management needed.
