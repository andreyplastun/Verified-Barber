...
- **Phone-Centric Anti-Spam (SINGLE CONVERSATION GUARANTEE)**: 1 phone = max 1 message per 20h window. Multi-layer protection:
  - **Layer 1 (Enqueue gate)**: At enqueue time, if phone is in 20h cooldown (already sent) → NOT enqueued. If phone has existing queued primary → keeps only freshest visit, supersedes rest (`skipped_superseded`).
  ...
  - **Layer 3 (Batch SELECT)**: `DISTINCT ON (customer_phone)` guarantees max 1 message per phone per batch cycle. Priority: reminder > primary.
  - **Layer 4 (In-batch set)**: `sentPhonesThisBatch` Set catches any edge cases from DISTINCT ON.
  - **Layer 5 (Phone lock)**: PostgreSQL advisory lock (`pg_try_advisory_lock`) acquired before ANY send. TTL=30s. If lock not acquired → reschedule +2min. Prevents race conditions across batches/instances.
  - **Layer 6 (Final phone guard)**: Inside lock, BEFORE actual send: re-checks if phone received message in last 20h. If yes → `skipped (final_phone_guard)`. This is the LAST line of defense — `send()` NEVER fires if guard fails.
