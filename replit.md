...
- **Visit Lifecycle**: Bookings follow a 5-status lifecycle: `scheduled` → `ready_to_complete` (auto after 20min grace) → `payment_pending` or `completed`. Two completion paths: "Request Payment" (→ payment_pending, +2 score on payment) and "Send Review" (→ completed, +1 score). Cancel allowed only from scheduled/ready_to_complete. Background jobs run every 5min for status transitions and NOT_COMPLETED flagging (24h in ready_to_complete). Negative motivation dampening: NOT_COMPLETED ≥3 in 7d → 0.8x trustedRating, ≥5 → 0.6x.
- **Booking & Payment Flow**: Payment confirmation (via webhooks) transitions payment_pending → completed and triggers magic link creation. The system handles refunds, ensures idempotency for payment/refund events, flags uncompleted bookings.
