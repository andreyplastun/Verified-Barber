...
- **Phone-Centric Anti-Spam (SINGLE CONVERSATION GUARANTEE)**: 1 phone = max 1 message per 20h window. Multi-layer protection:
  - **Layer 1 (Enqueue gate)**: At enqueue time, if phone has existing queued primary → keeps only freshest visit, supersedes rest (`skipped_superseded`). NO cooldown skip at enqueue — all primaries are always created.
  - **Layer 2 (Pre-batch dedup)**: `deduplicateQueueByPhone()` runs before each batch. Groups by phone+type, keeps best message (newest visit, highest price), supersedes rest (`superseded_phone_dedup`).
  - **Layer 3 (In-batch set)**: `sentPhonesThisBatch` Set guarantees max 1 message per phone per batch cycle. Duplicates deferred +20h.
  - **Layer 4 (Phone lock)**: PostgreSQL advisory lock (`pg_try_advisory_lock`) acquired before ANY send. TTL=30s. If lock not acquired → reschedule +2min. Prevents race conditions across batches/instances.
  - **Layer 5 (Phone cooldown in send)**: Inside lock, BEFORE actual send: checks if phone received message in last 20h. If yes → RESCHEDULE to lastSentAt + 20h (NOT skip). Messages are never lost to cooldown.
  - **ENV guard**: `sendViaAssistBot()` blocked in non-production (`IS_PRODUCTION` check). Dev/staging cannot send real messages.
  - **Followup superseding**: `createFollowup()` cancels any existing queued reminders for same phone before creating new one.
  - **Specialist bypass**: `sendDirectWaMessage()` (specialist actions) bypasses ALL phone-centric limits — always sends immediately.
- **Batch Processing (overfetch + retry loop)**: Processor fetches candidates with 3x overfetch (`available * 3`), processes one-by-one with phone dedup in code. Up to 5 rounds per cycle to compensate for skips/defers. Breaks when no more candidates or no sends in a round. Ensures daily limit is reached if enough messages exist.
