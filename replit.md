...
- **Review Weight System (Этап 1)**: Multi-factor review weighting stored in `review_geodata`. Factors: `geo_weight` (distance-based), `new_weight` (unique clients per 7 days, EXCLUDING current phone: ≥3→1.0, 2→0.8, ≤1→0.5), `repeat_weight` (binary: 1 review per phone per specialist per 180 days → 0, else 1; raised from 60d in 2026-07). For **manual**: `final_weight = geo_weight * new_weight * repeat_weight`. For **altegio**: `final_weight = 1.0` if repeat_weight=1, else 0 (no geo/new/text applied). If any weight=0 → final_weight=0. Rating uses `finalWeight` from geodata. Functions in `server/antifraud.ts`.
- **Daily Booking Creation Limit (2026-07)**: Anti-burst cap on non-Altegio bookings per specialist per Almaty day. Base 5/day; ≥25 in trailing 7 days → 7/day; ≥40 last week AND ≥25 previous week → 9/day (hard ceiling <10). Enforced in both POST /api/specialist/bookings and public POST /api/bookings via shared `checkManualDailyLimit` (routes.ts); counts booking_source IN (specialist_manual, client_app) OR NULL. 429 + `dailyLimitReached` → modal in SpecialistDashboard. Public endpoint now stamps bookingSource=client_app. Phone review cooldown (`calculateRepeatWeight`) raised 60 → 180 days. **Manual-booking 0.3 penalty (Altegio bypass)** applies only if Altegio data actually flows (`specialistHasWorkingAltegio`: ≥1 booking_source=altegio); nominal connection (company_id saved, salon never installed app, zero webhooks) → 0.6 like plain manual.
- **Geofencing System**: Location-based review trust via specialist's own `workAddress`/`workLat`/`workLng` fields. Specialist sets address in profile (text input + geocode, or "determine my location" GPS button). Nominatim (OpenStreetMap) geocoding. Haversine distance from reviewer to specialist's work address. **Geo is a BONUS, never a penalty** (decided 2026-06-16): confirmed on-site presence (≤200m) → `geo_weight = 1.05`; everything else (far / no GPS / no permission) → `geo_weight = 1.0` neutral. Rationale: legit clients often review later from home (far/no GPS), so absence of location must NOT reduce weight. **Time penalty REMOVED** (penalized busy honest clients + contradicted our own ~24h followup; Yandex invites reviews 2 days later — late ≠ fraud). **IP penalty REMOVED** (IP is shared via CGNAT/salon WiFi → false positives; one-person spam already fully closed by `repeat_weight` phone dedup). IP still recorded in `review_geodata.ipAddress` for monitoring only, no weight effect. Frontend geolocation request on MagicReviewPage with trust badge. Admin "Точки" tab removed — no manual location management needed.
- **"Рядом со мной" Near-Me Sort**: Top-level geo-sort toggle on the home specialist list (`client/src/pages/index.tsx`), placed ABOVE the collapsible Фильтры panel. On tap requests browser geolocation, sorts specialists ascending by Haversine distance from user to each specialist's `workLat`/`workLng`; specialists without coords sink to the bottom (`Infinity`) and show "адрес не указан". Distance shown per card ("X м" / "X.X км"). Pure client-side (list API already returns coords) — no backend change. Choosing a normal sort button clears geo mode. **Address adoption push** (because almost no specialists had addresses): amber banner in `SpecialistDashboard` when `workLat`/`workLng` missing (scrolls to + focuses `#workAddress`), plus a weight-0 optional `address` activation step in `shared/activation.ts` (total activation weight stays 100; never blocks the score).
