...
**Counter integrity:** "Sent today" in admin counts `status='sent'` which only means "AssistBot returned HTTP 200". Real delivery lives in `delivery_status = 'delivered'` from the callback above. Always surface both numbers (sent vs delivered) so provider-side silent failures are visible — yesterday's incident: 35 marked sent, 0 actually delivered to WhatsApp, no signal in UI.
