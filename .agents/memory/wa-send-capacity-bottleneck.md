...
**Follow-up on/off is a runtime toggle, not a code change:** WhatsApp reminders (messageType `reminder`) can be switched off from Admin → WhatsApp tab via `appConfig` key `WA_FOLLOWUP_ENABLED` (default ON; only literal `"false"` disables). **Why:** under primary-send scarcity, followups displace primaries — admin needs to kill them instantly without a deploy, then re-enable once the cap is raised. **How to apply:** disabling must stop BOTH new creation and the already-queued backlog, else old reminders keep draining send slots for ~24h; queued reminders are marked skipped (`followup_disabled`). Primaries are never affected by this flag. No schema work — appConfig is key-value.
