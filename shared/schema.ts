...
export const waMessages = pgTable("wa_messages", {
  ...
  deliveryReceivedAt: timestamp("delivery_received_at"),
  deliveryRaw: text("delivery_raw"),
  createdAt: timestamp("created_at").defaultNow(),
});
...
export const reviewGeodata = pgTable("review_geodata", {
  ...
  newWeight: real("new_weight").default(1.0).notNull(),
  repeatWeight: real("repeat_weight").default(1.0).notNull(),
  finalWeight: real("final_weight").default(1.0).notNull(),
  reviewSource: text("review_source", { enum: ["altegio", "manual"] }).default("manual").notNull(),
